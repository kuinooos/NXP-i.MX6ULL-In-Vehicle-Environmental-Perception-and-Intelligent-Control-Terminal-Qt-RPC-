...
/*初始化服务端套接字*/
static int init_Socket_imx6ull(){
	int iRet;
...
void weather_read(char** text, char** code,char** temperature){
	/*解引用后是地址*/
	*text = strdup(text_1.c_str());          
    *code = strdup(code_1.c_str());
    *temperature = strdup(temperature_1.c_str());
};
